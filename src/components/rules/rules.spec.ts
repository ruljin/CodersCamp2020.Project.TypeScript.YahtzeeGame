...
test('Check rendering text with rules', () => {
  const rules = createElementFromString(`<main class="box">
        <p>The game consists of a number of rounds. In each round, a player gets three rolls of the dice,
        ...
        <p>The Yahtzee scorecard contains 13 different category boxes and in each round, after the third
        ...
        <p>In the upper section there are six boxes. The score in each of these boxes is determined by
        ...
        <p>For the six combinations above the score for each of them is the sum of dice of the right kind. E.g. if you
        ...
        <p>A Yahtzee occurs when all five dice are the same. If the player throws a Yahtzee and has already filled the
